...
ok('vẫn hiện đúng quy cách 500 g đang bật cho Mùn cưa (23/09/2026: nay bật/tắt được theo Danh mục)', /class="h on" data-drqc="\d+\|\|\|500 g"/.test(zVeDR.html));
const zQC=await Ea(async i=>{const cho=ms=>new Promise(z=>setTimeout(z,ms)), p=sp[i], goc={dr:[...dr],dauRa:[...p.dauRa],pb:JSON.stringify(p.pb||[])};
  if(!dr.includes('10kg'))dr.push('10kg'); ve(); await cho(40);
  const kq={hien:!!document.querySelector('[data-drqc="'+i+'|||10kg"]')};
  document.querySelector('[data-drqc="'+i+'|||10kg"]').click(); await cho(40);
  kq.bat=p.dauRa.includes('10kg')&&p.dauRa.includes('500 g');
  document.querySelector('[data-drqc="'+i+'|||500 g"]').click(); await cho(40);
  kq.tat=!p.dauRa.includes('500 g')&&!(p.pb||[]).some(d=>d.qc==='500 g');
  document.querySelector('[data-drqc="'+i+'|||10kg"]').click(); await cho(40);
  kq.chanHet=p.dauRa.length===1&&p.dauRa[0]==='10kg';
  dr.length=0; goc.dr.forEach(x=>dr.push(x)); p.dauRa=goc.dauRa; p.pb=JSON.parse(goc.pb); luu(); ve();
  return kq},zDR);
ok('quy cách mới thêm ở Danh mục (10kg) hiện ngay trong Công thức', zQC.hien, JSON.stringify(zQC));
ok('bấm để bật quy cách mới cho mặt hàng', zQC.bat);
ok('bấm để tắt quy cách → gỡ luôn dòng đóng gói của quy cách đó', zQC.tat);
ok('không cho tắt quy cách cuối cùng (mặt hàng phải có ít nhất 1)', zQC.chanHet);
