...
/* R15 (10/09/2026) — màu in tem: chọn trong bảng Dàn trang, xem trước ngay, lưu theo máy, bản in + file in đúng màu */
const s22=await E(async()=>{ window.__tatTuDongKhoTem=true; const cho=ms=>new Promise(z=>setTimeout(z,ms)), kq={};
  ...
  const r={id:'S22-T',sp:p.ten,huong:'H',quyCach:'1 kg',sl:10,soBao:10,kgBao:1,ngaysx:'2026-09-10',lot:'1101070926',nv:'T',daGui:1}; nk.push(r);
  ...
  return kq;});
ok('R15: mặc định vẫn in đen như cũ', s22.macDinh, JSON.stringify(s22));
ok('R15: bảng Dàn trang có mục Màu in (3 phần × 6 màu có sẵn + màu tự chọn)', s22.coMuc);
ok('R15: chọn màu tên sản phẩm → xem trước đổi màu ngay, phần khác giữ nguyên', s22.xemTruoc);
ok('R15: màu tự chọn cho viền & kẻ bảng áp vào xem trước', s22.tuChon);
ok('R15: bấm In → nhớ màu đã chọn; bản in đúng màu từng phần', s22.chuaLuu&&s22.daLuu&&s22.banIn);
ok('R15: file in mang sang máy khác giữ đúng màu', s22.file);
ok('R15: Huỷ → không đổi màu đã lưu; mở lại bảng thấy đúng màu đang dùng', s22.huy&&s22.moLaiDungMau);
